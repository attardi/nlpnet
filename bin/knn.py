...
def read_vocabulary(input_file):
  ...
  dict = {}
  for line in fh:
    dict[line.strip().decode('utf-8')] = len(dict)
  return dict
...
Show knn of words typed on stdin."""
  ...
  word_id = read_vocabulary(vocabfile)
  # Map indices to words
  id_word = [None] * len(word_id)
  for v,i in word_id.iteritems():
      id_word[i] = v
  embeddings = loadEmbeddings(file)
  ...
  if options.kmeans:
    ...
    annotate(embeddings, id_word, word_id, options.annotate, options.col)
  else:
